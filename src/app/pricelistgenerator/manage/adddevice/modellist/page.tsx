...
function LFPTable({
  ...
  items: PlotterHardware[];
  loading: boolean;
  onEdit: (item: PlotterHardware) => void;
}) {
  if (loading) {
    return (
    ...
    <table className="w-full text-left">
      <thead>
        <tr className="border-b border-slate-100 bg-slate-50/70">
          ...
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400">Code</th>
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 text-right">Net (RM)</th>
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 text-right">Market (RM)</th>
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400">Warranty</th>
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400">Remark</th>
          <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 text-center w-20">Action</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item, idx) => (
          <tr key={item.id}
            className={`group border-b border-slate-100 last:border-0 hover:bg-emerald-50/30 transition-colors ${idx % 2 !== 0 ? 'bg-slate-50/30' : ''}`}>
            ...
            <td className="px-5 py-3.5">
              {item.productType ? (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-emerald-50 text-emerald-700 border-emerald-100 uppercase">
                  {item.productType}
                </span>
              ) : (
            ...
            <td className="px-5 py-3.5 text-right">
              <span className="text-sm font-bold text-blue-600 tabular-nums">RM {fmt(item.netPrice)}</span>
            </td>
            <td className="px-5 py-3.5 text-right">
              <span className="text-sm font-semibold text-emerald-600 tabular-nums">RM {fmt(item.marketPrice)}</span>
              {item.marketPrice > item.netPrice && (
                <p className="text-[10px] text-emerald-400 tabular-nums text-right">
                  +RM {fmt(item.marketPrice - item.netPrice)}
                </p>
              )}
            </td>
            <td className="px-5 py-3.5">
              <span className="text-xs text-slate-600">{item.warranty || <span className="text-slate-300">—</span>}</span>
            </td>
            <td className="px-5 py-3.5 max-w-[160px]">
              <p className="text-xs text-slate-500 truncate">{item.remark || <span className="text-slate-300">—</span>}</p>
            </td>
            <td className="px-5 py-3.5 text-center">
              <button
                onClick={() => onEdit(item)}
                className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50"
              >
...
function LFPEditModal({
  item,
  baseUrl,
  onClose,
  onSaved,
}: {
  item: PlotterHardware;
  baseUrl: string;
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  const set = (field: keyof PlotterHardware) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${baseUrl}/api/PlotterHardware/${item.id}`, {
        method: 'PUT',
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`${baseUrl}/api/PlotterHardware/${item.id}`, { method: 'DELETE' });
      if (res.ok) { onSaved(); onClose(); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <form onSubmit={handleSubmit}
        className="relative bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden z-10"
        style={{ maxHeight: '90vh' }}>

        {/* Header */}
        <div className="bg-white border-b border-slate-100 px-6 py-5 flex items-start justify-between flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">Edit LFP Product</p>
              <p className="text-[11px] text-slate-400 font-mono">{item.productName}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 min-h-0">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Product Name <span className="text-red-400">*</span></label>
            <input type="text" required value={values.productName} onChange={set('productName')}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Product Code <span className="text-red-400">*</span></label>
              <input type="text" required value={values.productCode} onChange={set('productCode')}
                className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition"/>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Product Type</label>
              <select value={values.productType} onChange={set('productType')}
                className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-blue-500 uppercase tracking-wide mb-1.5">Net Price (RM)</label>
              <input type="number" step="0.01" min="0" value={values.netPrice} onChange={set('netPrice')}
            ...
            <div>
              <label className="block text-xs font-semibold text-emerald-500 uppercase tracking-wide mb-1.5">Market Price (RM)</label>
              <input type="number" step="0.01" min="0" value={values.marketPrice} onChange={set('marketPrice')}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Warranty</label>
            <input type="text" value={values.warranty ?? ''} onChange={set('warranty')}
              placeholder="e.g., 1 Year On-Site"
              className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-slate-50 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition"/>
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Remark</label>
            <textarea rows={3} value={values.remark ?? ''} onChange={set('remark')}
              placeholder="Any extra notes…"
              className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 bg-slate-50 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition resize-none"/>
          </div>
        </div>

        {/* Footer */}
        <div className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex items-center justify-end gap-2 flex-shrink-0">
          <button type="button" onClick={onClose}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-5 flex flex-col items-center text-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800">Delete "{item.productName}"?</h3>
                <p className="text-xs text-slate-500 mt-1 leading-relaxed">This permanently removes the LFP product record. Cannot be undone.</p>
              </div>
            </div>
...
export default function ModelListPage() {
  ...
  const [lfpModels,  setLfpModels]  = useState<PlotterHardware[]>([]);
  const [loading,    setLoading]    = useState(true);
  const [lfpLoading, setLfpLoading] = useState(true);
  const [activeTab,  setActiveTab]  = useState<'ALL' | 'Canon' | 'KM' | 'LFP'>('ALL');
  ...
  // ── LFP edit modal ────────────────────────────────────────
  const [editingLFP, setEditingLFP] = useState<PlotterHardware | null>(null);
  ...
  const fetchLfpModels = async () => {
    try {
      setLfpLoading(true);
      const res = await fetch(`${baseUrl}/api/PlotterHardware`);
      if (res.ok) setLfpModels(await res.json());
    } catch (err) { console.error(err); }
    finally { setLfpLoading(false); }
  };
  ...
  const openEditModal = async (item: CopierModel) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelectedEqIds((fullConfig.equipments || []).map((eq: any) => eq.id));
        setRules((fullConfig.rules || []).filter((r: any) => r.modelId !== null).map((r: any) => ({ equipmentIdA: r.equipmentIdA, equipmentIdB: r.equipmentIdB })));
      }
    } catch (err) { console.error(err); }
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    for (const r of rules) {
      if (r.equipmentIdA === r.equipmentIdB) { alert('A spec cannot conflict with itself!'); return; }
    }
    try {
      setSubmitting(true);
      const payload = {
        model: { modelName: formModelName, brand: formBrand, category: formCategory, type: formType, basicPrice: Number(formBasicPrice), price36Months: Number(formPrice36Months), price60Months: Number(formPrice60Months), selectedEquipmentIds: [], exclusionRules: [] },
        equipments: selectedEqIds.map(id => ({ id })),
        rules: rules.map(r => ({ equipmentIdA: r.equipmentIdA, equipmentIdB: r.equipmentIdB })),
      };
      const res = await fetch(`${baseUrl}/api/Copier/update-by-id/${selectedModelId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (res.ok) { setIsEditModalOpen(false); await fetchAllModels(); }
      else alert(`Update failed: ${await res.text()}`);
    } catch (err) { console.error(err); }
    finally { setSubmitting(false); }
  };
  ...
  // ── Derived ───────────────────────────────────────────────
  const filteredCopierModels = activeTab === 'ALL'
    ? models
    : models.filter(m => m.brand?.toUpperCase() === activeTab);
  ...
  const tabCounts: Record<string, number> = {
    ALL:   models.length,
    Canon: models.filter(m => m.brand?.toUpperCase() === 'CANON').length,
    KM:    models.filter(m => m.brand?.toUpperCase() === 'KM').length,
    LFP:   lfpModels.length,
  };
    ...
    <div className="min-h-screen bg-[#F1F5F9] font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-800 leading-tight">Model Inventory</h1>
      ...
      <div className="px-8 py-5 max-w-7xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            {(['ALL', 'Canon', 'KM', 'LFP'] as const).map(tab => (
          ...
          <div className="flex items-center gap-2">
            {activeTab === 'LFP' && (
              <span className="flex items-center gap-1.5 text-[11px] font-semibold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full">
                ...
                Large Format Printer — PlotterHardware table
              </span>
            )}
            <span className="text-[11px] text-slate-400">
              Showing <span className="font-semibold text-slate-600">
                {activeTab === 'LFP' ? lfpModels.length : filteredCopierModels.length}
              </span> records
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          {activeTab === 'LFP' ? (
            <LFPTable
              items={lfpModels}
              loading={lfpLoading}
              onEdit={setEditingLFP}
            />
          ) : (
            <CopierTable
              items={filteredCopierModels}
              loading={loading}
      ...
      {editingLFP && (
        <LFPEditModal
          item={editingLFP}
          baseUrl={baseUrl}
          onClose={() => setEditingLFP(null)}
          onSaved={fetchLfpModels}
        />
      )}
          ...
          <form onSubmit={handleUpdateSubmit}
            ...
            <div className="bg-white border-b border-slate-100 px-6 pt-5 pb-0 flex-shrink-0">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => { setDeleteModelName(originalModelName); setDeleteConfirmId(selectedModelId); }}
                    className="flex items-center gap-1 text-xs font-semibold text-red-500 border border-red-200 bg-red-50 hover:bg-red-100 px-2.5 py-1.5 rounded-lg transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7h6m-7 0a1 1 0 01-1-1V5a1 1 0 011-1h6a1 1 0 011 1v1a1 1 0 01-1 1H9z"/>
                    </svg>
              ...
              <div className="flex gap-1">
                {MODAL_TABS.map(t => (
                  <button key={t.key} type="button" onClick={() => setModalTab(t.key)}
                    className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold border-b-2 transition-colors ${modalTab === t.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    {t.label}
                    {t.count !== null && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${modalTab === t.key ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>{t.count}</span>
                    )}
                  </button>
                ))}
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <select value={formType} onChange={e => setFormType(e.target.value)}
                        className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition">
                        {formBrand === 'Canon' ? (
                          <><option value="A4 Desktop Series">A4 Desktop</option><option value="A3 Desktop Series">A3 Desktop</option><option value="A3 Light Duty Series">A3 Light Duty</option></>
                        ) : (
                          <><option value="Light Duty Series">Light Duty</option><option value="Mid-Heavy Duty Series">Mid-Heavy Duty</option><option value="Light Production Series">Light Production</option></>
                        )}
                      </select>
                    </div>
                  </div>
                  ...
                  {loadingSpec ? (
                    ...
                    <>
                      <div className="relative">
                        <svg className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <input type="text" placeholder="Search specs…" value={specSearch} onChange={e => setSpecSearch(e.target.value)}
                        ...
                        {Object.entries(groupByCategory(filteredSpecs)).map(([catName, specs]) => (
                          <div key={catName}>
                            ...
                            {specs.map(eq => {
                              ...
                              return (
                                <label key={eq.id} className={`flex items-center gap-3 px-4 py-3 border-b border-slate-50 last:border-0 cursor-pointer transition-colors ${checked ? 'bg-blue-50/50' : 'hover:bg-slate-50'}`}>
                    ...
                    <div className="space-y-2">
                      {rules.map((rule, idx) => (
                        <div key={idx} className="flex items-center gap-2.5 bg-red-50/60 border border-red-100 rounded-xl px-4 py-3">
                          ...
                          <svg className="w-4 h-4 text-red-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
            ...
            <div className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setIsEditModalOpen(false)}
                  className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors">Cancel</button>
                <button type="submit" disabled={submitting}
                  className="flex items-center gap-1.5 px-5 py-2 text-sm font-bold bg-amber-500 hover:bg-amber-400 disabled:bg-amber-300 text-white rounded-xl shadow-sm transition-colors">
                  {submitting
                    ? <><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Saving…</>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-5 flex flex-col items-center text-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1 leading-relaxed">This permanently removes the model, all equipment mappings, and conflict rules. Cannot be undone.</p>
              </div>
            </div>
            <div className="px-6 pb-5 flex gap-2">
