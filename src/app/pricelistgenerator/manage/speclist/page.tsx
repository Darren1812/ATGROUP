...
function getCategoryStyle(category: string) {
  return CATEGORY_OPTIONS.find((c) => c.value === category)?.color ?? "bg-gray-100 text-gray-600";
}
...
export default function EquipmentAdminPage() {
  ...
  const fetchEquipments = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const sorted = [...data].sort((a: EquipmentSpec, b: EquipmentSpec) => a.id - b.id);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!specName.trim()) { alert("Equipment name is required."); return; }
    const payload = { id: currentId, specName, category, basicPrice: Number(basicPrice), price60Months: Number(price60Months), price36Months: Number(price36Months) };
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || "";
      const url    = isEditing ? `${baseUrl}/api/EquipmentSpec/${currentId}` : `${baseUrl}/api/EquipmentSpec`;
      const method = isEditing ? "PUT" : "POST";
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (res.ok) { setIsModalOpen(false); fetchEquipments(); }
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/EquipmentSpec/${id}`, { method: "DELETE" });
      if (res.ok) { setDeleteConfirmId(null); fetchEquipments(); }
      else { alert("Failed to delete equipment."); }
    } catch (error) { console.error("Error deleting data:", error); }
  };

  // Derived
  const filtered = activeFilter ? equipments.filter(e => e.category === activeFilter) : equipments;
  ...
  return (
    ...
    <div className="bg-white border-b border-slate-200 px-8 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h1 className="text-sm font-bold text-slate-800 leading-tight">Equipment Specs</h1>
          <p className="text-[11px] text-slate-400 leading-tight">Optional add-ons &amp; rental pricing</p>
        </div>
      </div>
      ...
      <main className="px-8 py-5 max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          >
            All
            <span className={`text-[11px] font-bold tabular-nums ${activeFilter === null ? "opacity-70" : "text-slate-400"}`}>
              {equipments.length}
            </span>
          </button>
          ...
          {CATEGORY_OPTIONS.map((cat) => {
            const count = equipments.filter(e => e.category === cat.value).length;
            ...
            return (
              ...
              >
                ...
                {!isActive && (
                  <span className={`w-1.5 h-1.5 rounded-full inline-block ${cat.color.split(" ")[0].replace("bg-", "bg-").replace("100","500")}`} />
                )}
                {cat.label}
                <span className={`text-[11px] font-bold tabular-nums ${isActive ? "opacity-70" : "text-slate-400"}`}>
                  {count}
                </span>
              </button>
            );
          ...
          {activeFilter && (
            <span className="text-xs text-slate-400 ml-1">
              — showing <span className="font-semibold text-slate-600">{filtered.length}</span> of {equipments.length}
            </span>
          )}
        </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-2">
              ...
              <p className="text-sm font-medium">
                {activeFilter ? `No items in "${activeFilter}"` : "No equipment yet"}
              </p>
              {!activeFilter && (
                <p className="text-xs text-slate-400">Click <strong>Add Equipment</strong> to get started.</p>
              )}
            </div>
          ) : (
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/70">
                  <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 w-16">#</th>
                  <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400">Equipment Name</th>
                  <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400">Category</th>
                  <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 text-right">Purchase (RM)</th>
                  ...
                  <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-widest text-slate-400 text-center w-24">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-sm font-semibold text-slate-800">{item.specName}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-semibold ${getCategoryStyle(item.category)}`}>
                        {item.category || "—"}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-medium text-slate-700 tabular-nums">
                        {item.basicPrice > 0
                          ? item.basicPrice.toLocaleString("en-MY", { minimumFractionDigits: 2 })
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-semibold text-orange-500 tabular-nums">
                        {item.price36Months > 0
                          ? item.price36Months.toLocaleString("en-MY", { minimumFractionDigits: 2 })
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-semibold text-emerald-600 tabular-nums">
                        {item.price60Months > 0
                          ? item.price60Months.toLocaleString("en-MY", { minimumFractionDigits: 2 })
                          : <span className="text-slate-300 font-normal">—</span>}
                      </span>
                    </td>
          ...
          {!loading && equipments.length > 0 && (
            <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <span className="text-xs text-slate-400">
                {activeFilter
                  ? <>Filtered: <span className="font-semibold text-slate-600">{filtered.length}</span> of {equipments.length} equipments</>
                  : <>Showing <span className="font-semibold text-slate-600">{equipments.length}</span> equipment{equipments.length !== 1 ? "s" : ""}, sorted by ID</>
                }
              </span>
              <span className="text-xs text-slate-400">
                IDs #{equipments[0]?.id} – #{equipments[equipments.length - 1]?.id}
              </span>
            </div>
          )}
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 overflow-hidden">
            <div className={`px-6 py-4 flex items-center justify-between border-b border-slate-100 ${isEditing ? "bg-blue-50" : "bg-slate-50"}`}>
              <div className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white ${isEditing ? "bg-blue-500" : "bg-slate-700"}`}>
                  {isEditing ? (
                ...
                <div>
                  ...
                  {isEditing && <p className="text-[11px] text-slate-400">ID: {String(currentId).padStart(3, "0")}</p>}
                </div>
              </div>
            ...
            <form onSubmit={handleFormSubmit} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input
                  type="text" required placeholder="e.g., Inner Finisher-K1"
                  ...
                  {CATEGORY_OPTIONS.map((cat) => (
                    <button key={cat.value} type="button" onClick={() => setCategory(cat.value)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                        category === cat.value ? "border-blue-400 bg-blue-500 text-white shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-slate-300"
                      }`}>
                      {cat.label}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Purchase (RM)</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-orange-400 uppercase tracking-wide mb-1.5">36 Mo /mo</label>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-5 flex flex-col items-center text-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800">Delete Equipment?</h3>
                <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                  This will permanently remove ID <span className="font-mono font-semibold text-slate-700">{String(deleteConfirmId).padStart(3, "0")}</span> and any linked pricing rules. This cannot be undone.
                </p>
              </div>
            </div>
