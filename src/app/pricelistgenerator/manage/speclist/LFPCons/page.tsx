                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">
                    ...
                    {selectedHardwareIds.length > 0 && (
                      <span className="ml-2 text-[10px] font-bold text-slate-900 bg-slate-100 px-1.5 py-0.5 rounded-full normal-case">{selectedHardwareIds.length} selected</span>
                    )}
                  </label>
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex flex-wrap gap-2 min-h-[52px]">
                    {hardwareOptions.length === 0 ? (
                      <p className="text-xs text-slate-400 italic self-center">No hardware products found.</p>
                    ) : hardwareOptions.map(hw => {
                      const sel = selectedHardwareIds.includes(hw.id);
                      return (
                        <button key={hw.id} type="button" onClick={() =>
                          setSelectedHardwareIds(prev => sel ? prev.filter(id => id !== hw.id) : [...prev, hw.id])}
                          className={`text-xs px-3 py-1.5 rounded-lg font-semibold transition-all border ${
                            sel ? "bg-slate-900 border-slate-900 text-white shadow-sm" : "bg-white border-slate-200 text-slate-600 hover:border-slate-400"
                          }`}>
                          {hw.displayName}
                        </button>
                      );
