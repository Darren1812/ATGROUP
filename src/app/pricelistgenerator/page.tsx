...
export default function PriceListGeneratorPage() {
  ...
  useEffect(() => {
    ...
    if (selectedBrand === 'Canon') {
        setSelectedType('A4 Desktop Series');
    } else {
        setSelectedType('Light Duty Series');
    }
    }, [selectedBrand]);
  ...
  useEffect(() => {
    ...
    fetch(
    `${baseUrl}/api/Copier/generator-data?brand=${selectedBrand}&category=${selectedCategory}&type=${encodeURIComponent(selectedType)}`
    )
      .then((res) => res.json())
      .then((data) => { setFetchedData(data); setLoading(false); })
      .catch((err) => { console.error(err); setLoading(false); });
  }, [selectedBrand, selectedCategory, selectedType, baseUrl]);
  ...
  const isSpecDisabled = (specId: number) => {
    if (selectedSpecIds.includes(specId)) return false;
    ...
    <div className="min-h-screen bg-[#F1F5F9] font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 leading-tight">Configure options and generate real-time quotations</p>
          </div>
        </div>
      ...
      <div className="px-8 py-5 max-w-7xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => router.push('/pricelistgenerator/manage/adddevice')}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-800">Create New Copier Model</p>
              <p className="text-xs text-slate-400">Add machines, costs, 36 / 60-month packages</p>
            </div>
          ...
          <button onClick={() => router.push('/pricelistgenerator/manage/speclist')}
            className="group flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-left hover:border-orange-300 hover:shadow-sm transition-all">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-800">Optional Specs &amp; Global Rules</p>
        ...
        {loading ? (
          ...
          <>
            {/* ══ Quotation Summary Bar ═══════════════════════════════
                Sits ABOVE the configurator, always visible             */}
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  {currentPack && (
                    <span className="text-xs font-bold text-white opacity-80">— {currentPack.model.modelName}</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 divide-x divide-slate-100">
                ...
                <div className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-blue-600 tabular-nums leading-none">
                      RM {fmt(totals.outright)}
                    </p>
              ...
              {/* Selected add-ons receipt strip (only when items chosen) */}
                ...
                <div className="border-t border-slate-100 bg-slate-50/60 px-6 py-3 flex flex-wrap gap-2 items-center">
                  ...
                  {currentPack.equipments.filter(s => selectedSpecIds.includes(s.id)).map(spec => (
                    <span key={spec.id}
                      ...
                      {spec.specName}
                      <button onClick={() => handleSpecToggle(spec.id)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 items-start">
              ...
              {/* Left column: model picker + base pricing */}
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-slate-800 truncate">
                              {currentPack?.model.modelName ?? 'Select a model'}
                            </p>
                            {currentPack && (
                              <p className="text-[11px] text-slate-400">{currentPack.model.brand} · {currentPack.model.category}</p>
                            )}
                          </div>
                        </div>
                        <svg className={`w-4 h-4 text-slate-400 flex-shrink-0 transition-transform ${modelDropOpen ? 'rotate-180' : ''}`}
                            ...
                            {fetchedData.map((pack, idx) => (
                              ...
                              >
                                <div className="flex items-center gap-3 min-w-0">
                                  ...
                                  <span className={`text-sm font-semibold truncate ${activeModelIdx === idx ? 'text-blue-700' : 'text-slate-700'}`}>
                                    {pack.model.modelName}
                                  </span>
                                </div>
                                <span className="text-xs tabular-nums text-slate-400 flex-shrink-0 ml-4">
                                  RM {fmt(pack.model.basicPrice)}
                                </span>
                              </button>
                            ))}
                          </div>
                    ...
                    <div className="divide-y divide-slate-100">
                      <div className="flex items-center justify-between px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-slate-600">Outright Purchase</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-slate-600">36-Month Rental</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-slate-600">60-Month Rental</span>
                        </div>
              ...
              {/* Right column: Optional specs */}
              {currentPack && (
                <div className="lg:col-span-3">
                  <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    {currentPack.equipments.length === 0 ? (
                      <div className="py-14 text-center text-slate-400">
                        <p className="text-sm">No optional specs mapped to this model.</p>
                      </div>
                    ) : (
                      <div>
                        {Object.entries(groupByCategory(currentPack.equipments)).map(([catName, specs]) => (
                          <div key={catName}>
                            ...
                            <div className="px-5 py-2 bg-slate-50 flex items-center gap-2 border-b border-slate-100">
                              ...
                              <span className={`ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded ${CAT_STYLE[catName]?.badge ?? 'bg-slate-100 text-slate-400'}`}>
                                {specs.length}
                              </span>
                            </div>
                            {specs.map((spec) => {
                              ...
                              return (
                                <label key={spec.id}
                                  className={`flex items-center gap-4 px-5 py-3.5 border-b border-slate-50 last:border-0 transition-colors ${
                                    checked   ? 'bg-blue-50/50 cursor-pointer' :
                                    disabled  ? 'opacity-40 cursor-not-allowed bg-slate-50/50' :
                                  ...
                                  <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                                    ...
                                    <input type="checkbox" checked={checked} disabled={disabled}
                                      onChange={() => handleSpecToggle(spec.id)} className="sr-only"/>
                                    {checked && (
                                  ...
                                  {/* Name */}
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-semibold truncate ${checked ? 'text-blue-700' : disabled ? 'text-slate-400' : 'text-slate-800'}`}>
                                      {spec.specName}
                                    </p>
                                    {disabled && (
                                      <p className="text-[11px] text-red-400 font-medium mt-0.5">Conflict with a selected item</p>
                                    )}
                                  </div>
                                  ...
                                  <div className="hidden sm:flex items-center gap-5 flex-shrink-0">
                                    <div className="text-right w-24">
                                      <p className="text-[10px] text-slate-400 mb-0.5">Purchase</p>
                                      <p className={`text-xs font-semibold tabular-nums ${checked ? 'text-blue-600' : 'text-slate-600'}`}>
                                        {spec.basicPrice > 0 ? `+RM ${fmt(spec.basicPrice)}` : '—'}
                                      </p>
                                    </div>
                                    <div className="text-right w-20">
                                      <p className="text-[10px] text-amber-400 mb-0.5">36 mo</p>
                                      <p className={`text-xs font-semibold tabular-nums ${checked ? 'text-amber-500' : 'text-slate-500'}`}>
                                        {spec.price36Months > 0 ? `+RM ${fmt(spec.price36Months)}` : '—'}
                                      </p>
                                    </div>
                                    <div className="text-right w-20">
                                      <p className="text-[10px] text-emerald-400 mb-0.5">60 mo</p>
                                      <p className={`text-xs font-semibold tabular-nums ${checked ? 'text-emerald-600' : 'text-slate-500'}`}>
                                        {spec.price60Months > 0 ? `+RM ${fmt(spec.price60Months)}` : '—'}
                                      </p>
                                    </div>
                                  </div>
                                </label>
                    ...
                    <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                      <p className="text-[10px] text-slate-400">
                        {currentPack.equipments.length} specs available · {currentPack.rules.length} conflict rules active
                      </p>
