...
export default function ARENA() {
  ...
  const FUNCTION_MODEL_SUFFIX: Record<string, string> = {
    Fax: "WITH FAX",
    Inner: "WITH INNER FINISHER",
    Booklet: "WITH EXTERNAL BOOKLET FINISHER",
    Puncher: "WITH PUNCHER",
    Staple: "WITH EXTERNAL STAPLE FINISHER",
  };
  ...
  const handleFunctionToggle = (index: number, func: string) => {
    ...
    // ===================== BUILD i_model (EDITABLE DISPLAY NAME) =====================

    // IMPORTANT: use base model (fallback to avoid stacking suffix repeatedly)
    const baseModel =
      item.modelname ||
      item.baseModel ||
      item.i_model?.split(" WITH ")[0] ||
      "";

    // Build suffix string
    const suffixText = newSelected
      .map((f) => FUNCTION_MODEL_SUFFIX[f])
      .filter(Boolean);

    // Final display model
    const finalModel =
      suffixText.length > 0
        ? `${baseModel} ${suffixText.join(" ")}`
        : baseModel;

    // Save back
    handleItemChange(index, "i_model", finalModel);
    // --- Image update ---
                  ...
                  <div className='ml-10 bg-gradient-to-br from-blue-300 to-blue-50 border-2 border-blue-200/50 rounded-xl p-6 shadow-lg'>
                    ...
                    <div className='w-150 bg-white border border-gray-300 p-8 shadow-inner overflow-hidden text-xs space-y-3'>
                      ...
                      <p className='pt-4 text-gray-700'>
                        ...
                        model, we have realized an ambitious goal. Now small
                        departments and workgroups can enjoy stunningly
                        simplified and productive workflow... all in a compact
                        footprint.
                      </p>

                      <p className='pt-4'>
                        We are confident our proposed solution will meet your
                        organization’s immediate and future requirements. We
                        shall look forward to your approval on the
                        implementation of our proposed solution by{" "}
                      ...
                      <div className='mt-2 flex flex-col rounded-lg overflow-hidden'>
                        ...
                        <div className='flex flex-col text-sm'>
                          ...
                          <div className='flex border-l border-r border-b border-blue-400 text-black'>
                            ...
                            <p className='w-1/2 p-2'>{item.warm_Up || "-"}</p>
                          </div>
                          ...
                          <h2 className='font-bold text-l mt-5'>
                            Pricing Schedule
                          </h2>
                          ...
                          <div className='flex border-r border-l border-b border-blue-400'>
                            ...
                            <p className='w-1/2 p-2 text-black'>{item.mrpv}</p>
                          </div>
                        </div>
                      </div>
                    </div>
