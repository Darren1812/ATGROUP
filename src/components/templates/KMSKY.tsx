...
export default function SKY() {
  const generateFileName = (request: ReturnType<typeof generateJSON>) => {
    ...
    const modelNames = request.Items.map(
      (item) => item.i_model?.split(" ").pop() || "",
    ).filter(Boolean);
    const modelsPart = modelNames.join("_");

    const basicFunctions = ["Copy", "Print", "Scan", "Store"];
    const additionalFunctions = request.Items.flatMap((item) =>
      (item.i_function || "").split(",").map((f) => f.trim()),
    ...
    const templatePart =
      templateMap[request.TemplateName] || request.TemplateName;

    return (
      ["Proposal", templatePart, customerInitials, modelsPart, functionPart]
        .filter(Boolean)
        .join("_") + ".docx"
    );
  };
      ...
      setItems((prev) => {
        ...
        if (field === "rental_scheme") {
          ...
          updated = updated.map((item) => {
            ...
            const unitPrice =
              parseFloat(
                item.rental_unit?.toString().replace(/,/g, "") || "0",
              ) || 0;
        ...
        // 2. Logic for fields that affect ONLY the specific item
          ...
          if (field === "quantity" || field === "rental_unit") {
            const qty =
              parseFloat(currentItem.quantity?.toString() || "0") || 0;
            const unitPrice =
              parseFloat(
                currentItem.rental_unit?.toString().replace(/,/g, "") || "0",
              ) || 0;
            const duration =
              parseFloat(currentItem.rental_scheme?.toString() || "0") || 0;
  ...
  const [modelDetailCache, setModelDetailCache] = useState<
    Record<number, ModelDetail>
  >({});
    ...
    async (modelName: string, index: number) => {
      if (modelName.trim() === "") {
        ...
        [
          ...
          "paperWeights",
          "powerConsumption",
        ].forEach((field) => handleItemChange(index, field, ""));
      ...
      try {
        ...
        handleItemChange(
          index,
          "paperOutputCapacity_BF",
          data.paperOutputCapacity_BF || "",
        );
        handleItemChange(index, "stapling_BF", data.stapling_BF || "");
        handleItemChange(
          index,
          "paperOutputCapacity_SF",
          data.paperOutputCapacity_SF || "",
        );
        handleItemChange(index, "stapling_SF", data.stapling_SF || "");
        handleItemChange(
          index,
          "paperOutputCapacity_IF",
          data.paperOutputCapacity_IF || "",
        );
  ...
  useEffect(() => {
    if (featurePage) {
      setFeature1(featurePage);
      setFeature2(featurePage);
    } else {
      setFeature1("");
      setFeature2("");
    }
                    ...
                    <div className='space-y-4'>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value='imageRUNNER ADVANCE'>
                            imageRUNNER ADVANCE
                          </option>
                          <option value='imageRUNNER ADVANCE DX'>
                  ...
                  <div className='ml-10 bg-gradient-to-br from-blue-900 to-blue-50 border-2 border-blue-200/50 rounded-xl p-6 shadow-lg'>
                    ...
                    <div className='w-150 bg-white border border-gray-300 p-8 shadow-inner overflow-hidden text-xs space-y-3'>
                      ...
                      <p className='text-gray-700'>
                        SKY ACTIVE SDN BHD (1031890M) is pleased to enclose our
                        proposal outlining Canon{" "}
                        <span className='text-red-600 underline ml-2'>
                          {feature1 || "{feature1}"}
                        </span>{" "}
                        ’s recommendations for your in-house document production
                        solution. This proposed solution introduces highly
                        innovative technology implemented through the Canon
                        <span className='text-red-600 underline ml-2'>
                          {feature1 || "{feature1}"}
                        </span>{" "}
                        Series.
                      ...
                      <p className='pt-4 text-gray-700'>
                        Following an evaluation of the information you have
                        provided us concerning your current situation, I believe
                        that the Canon
                        <span className='text-red-600 underline ml-2'>
                          {feature1 || "{feature1}"}
                        </span>{" "}
                        Series will support your business and provide the
                        scalability necessary to enable your organization’s
                        growth in the future.
                      </p>

                      <p className='pt-4'>
                        This proposal incorporates information about your key
                        requirements, detailed pricing information and some key
                        Canon{" "}
                        <span className='text-red-600 underline ml-2'>
                          {feature1 || "{feature1}"}
                        </span>{" "}
                        Series features. It also offers superior value through
                        its customizable workflows, high image quality, advanced
                        features, user-friendly layout, low operating costs and
                        improved productivity.
                      </p>

                      <p className='pt-4'>
                        Thank you for taking the time to consider SKY ACTIVE SDN
                        BHD (1031890M) and Canon. I trust that you will find the
                        proposed solution meets your business requirements.
                      </p>
                      <p className='pt-4'>
                        Should you have any questions or additional
                        requirements, please do not hesitate to contact me on
                        012-736 3934 or via email at ansel_new@hotmail.com.
                      </p>
                    ...
                    <div className='ml-10 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200/50'>
                      ...
                      <div>
                        ...
                        <p className='text-xs text-green-700 mt-1'>
                          Keperluan PTJ: ( ) unit Mesin Penyalin Hitam/Putih
                          dengan berkelajuan ( ) ppm
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className='text-xs text-red-700 mt-1'>
                          Tawaran Pembekal: ( ) unit Mesin Penyalin Hitam/Putih
                          dengan berkelajuan ( )ppm, dilengkapi dengan fungsi
                          staple.
                        </p>
                      </div>
                      <div className='space-y-4'>
                        ...
                        <div className='mt-4'>
                          ...
                          <ReactSelect
                            ...
                            options={KM_MODEL_LIST.map((model) => ({
                              label: model.name,
                            ...
                            {[
                              ...
                              return (
                                <button
                                  ...
                                  type='button'
                                  onClick={() =>
                                    handleFunctionToggle(index, func)
                                  }
                                  className={`px-3 py-1 rounded-lg border text-sm font-semibold transition-all duration-200 focus:outline-none ${
                                    selected
                        ...
                        <div>
                          ...
                          >
                            ...
                            {item.threeTrays && (
                              <option value={item.threeTrays}>
                                {item.threeTrays}
                              </option>
                            )}
                            {item.fiveTrays && (
                              <option value={item.fiveTrays}>
                                {item.fiveTrays}
                              </option>
                            )}
                          </select>
                      ...
                      <div className='mt-2 flex flex-col rounded-lg overflow-hidden '>
                        ...
                        <div className='flex flex-col text-sm '>
                          ...
                          <div className='flex border-l border-r border-white text-black'>
                            ...
                            <p className='w-1/2 p-2 bg-blue-400'></p>
                          </div>
                          <div className='flex border border-white'>
                          ...
                          <h2 className='font-bold text-l mt-5 text-white'>
                            TAWARAN HARGA
                          </h2>
                          <div className='flex border border-white'>
                              ...
                              <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-3'>
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    value={
                                      item.HargaBulanan !== undefined &&
                                      item.HargaBulanan !== null
